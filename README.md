...
A cor de cada linha é decidida pelo texto exato que a API manda em `situacaoDescricao` (tabela `CORES_POR_SITUACAO` em `backend/src/mappers.ts`, espelhada em `frontend/src/lib/situacoes.ts` pra legenda), não por um código numérico. Confirmado contra a API real (25-26/08/2026): `Gerada`, `Recebida Qualidade`, `Em Inspeção`, `Liberada Qualidade`, `OF Baixada`. **Ainda não observado**: o status de reprovação (código `4`) — nenhuma OF passou por ele até agora, então registramos as duas grafias possíveis (`Reprovado` e `Reprovada`) apontando pra roxo, já que "Recebida" (e não "Recebido") mostrou que o gênero da palavra não é previsível. Se a API mandar outra coisa (ex.: com espaço diferente), a linha cai no cinza de fallback — é só adicionar a grafia certa na tabela.
